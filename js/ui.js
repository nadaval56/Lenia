...
import { Lenia, classifyState, KERNEL_TYPES } from './lenia.js?v=10';
import { LeniaMulti } from './multi.js?v=10';
import { Renderer, PALETTES, CHANNEL_COLORS } from './render.js?v=10';
import { PhaseMap } from './phasemap.js?v=10';
import { CREATURES } from './creatures.js?v=10';
...
const SUBJECTS = [
  {
    ...
    body: `<p>כל מה שאתם רואים על המסך הוא <strong>משוואה אחת</strong> שרצה שוב ושוב.
      בכל צעד, כל תא מחשב "כמה צפוף סביבי" (זו <strong>קונבולוציה</strong> — סכום
      משוקלל של השכנים), ואז שואל פונקציית פעמון גאוסי אם הצפיפות נעימה לו. אין שום
      "קוד" שמצייר את היצורים — יש נוסחה בת שורה אחת, וזהו.</p>
      <p>זו דוגמה חיה ל<strong>התהוות (emergence)</strong>: תופעה מורכבת שאי אפשר לנחש
      אותה מתוך הכללים הפשוטים שיוצרים אותה. מתמטיקאים אוהבים את לניה כי היא גם
      <strong>רציפה</strong> (ערכים בין 0 ל‑1, לא רק חי/מת) וגם <strong>גזירה</strong> —
      אפשר לחשב איך שינוי זעיר בפרמטר משפיע, וזה בדיוק מה שמחבר אותה לרשתות נוירונים
      מודרניות.</p>
      <p><strong>דוגמה ספציפית:</strong> הזיזו את μ מ‑0.15 ל‑0.16 — שינוי של אחוז אחד.
      האורביום שהיה יציב לגמרי מתחיל להתעוות ולהתפרק. מספר קטנטן, שינוי דרמטי: זו
      "רגישות לתנאי התחלה", לב תורת הכאוס.</p>
      <p class="help-try">🤔 למחשבה: איפה בדיוק "שמור" המידע על צורת האורביום? הוא לא
      כתוב בשום מקום — הוא מתקיים רק כתבנית שהמשוואה שומרת על עצמה. האם גם הזיכרון
      שלכם עובד ככה?</p>`,
  },
  {
    emoji: '🧬', name: 'ביולוגיה',
    body: `<p>האורביום מתנהג כמו <strong>תא חי בודד</strong>: יש לו "קרום" (הטבעת
      החיצונית הצפופה ששומרת על הפנים), הוא נע בכיוון קבוע, ומעל הכול — הוא שומר על
      עצמו. אם תפריעו לו קצת, הוא מתקן את עצמו וחוזר לצורתו. התכונה הזו נקראת
      <strong>הומאוסטזיס</strong>, וזה בדיוק מה שמחוון המסה מודד: תא חי הוא תא
      שמצליח להישאר "הוא עצמו" למרות שהעולם דוחף אותו כל הזמן.</p>
      <p>בתנאים מסוימים יצורי לניה אף <strong>מתחלקים לשניים</strong> — כמו חלוקת תא —
      או מתאחדים. הביולוגים משתמשים בלניה כדי לשאול: מהו המינימום ההכרחי לחיים?
      מסתבר שלא צריך DNA ולא חלבונים — מספיקים שלושה עקרונות: גבול, חילוף חומרים
      (גדילה/דעיכה), ושמירה עצמית.</p>
      <p><strong>דוגמה ספציפית:</strong> טענו את "ההיבלעות 🌀" — אורביום בודד שוחה
      לתוך מושבה. שימו לב לרגע שבו הוא מפסיק להיות פרט נפרד ונטמע בהמון. זה בדיוק
      הגבול המטושטש בין "יצור" ל"רקמה".</p>
      <p class="help-try">🤔 למחשבה: מה הופך משהו ל"חי"? אורביום נע, שומר על עצמו,
      נמנע מהתמוטטות ואף מתחלק. מה עוד צריך? ומה, אם בכלל, חסר לו?</p>`,
  },
  {
    emoji: '🌿', name: 'אקולוגיה',
    body: `<p>העולם הצבעוני "המרדף הגדול" הוא מודל <strong>טורף–נטרף</strong> אמיתי:
      הירוק (נטרף) חי בכוחות עצמו לפי חוק לניה רגיל, והאדום (טורף) בנוי כך שהוא יכול
      לגדול <em>רק</em> היכן שיש ירוק — אבל במקום שהוא גדל, הוא הורג אותו.</p>
      <p>מהחוק הפשוט הזה נולדים <strong>מחזורי אוכלוסייה</strong>: הרבה נטרף ⇐ הטורפים
      משגשגים ⇐ הם אוכלים יותר מדי ⇐ המזון נגמר ⇐ הטורפים קורסים ⇐ הנטרף מתאושש ⇐
      וחוזר חלילה. אלה בדיוק <strong>משוואות לוטקה–וולטרה</strong> שמתארות שועלים
      וארנבות, כרישים ודגים, ואפילו מגפות. גם <strong>סגרגציה מרחבית</strong> צצה
      לבד: הצבעים נמנעים זה מזה ויוצרים "טריטוריות" — בלי שאף אחד תכנת גבולות.</p>
      <p><strong>דוגמה ספציפית:</strong> פתחו את פאנל החוקים וגררו את ה‑h של החוק
      המדכא (הטורף→נטרף) לכיוון ‑1.5. הטורף הופך לרעיל מדי, מכלה את כל הירוק... ואז
      גווע ברעב בעצמו. שני הצבעים נעלמים. זו "קריסת מערכת אקולוגית" בשלושה גרירות.</p>
      <p class="help-try">🤔 למחשבה: הטורף לא "רוצה" לשמור על המזון שלו — הוא סתם
      מציית לחוק. ובכל זאת, טורף חמדן מדי מכחיד את עצמו. האם זה מלמד למה בטבע האמיתי
      טורפים כמעט אף פעם לא אוכלים את כל הטרף?</p>`,
  },
  {
    emoji: '💻', name: 'מדעי המחשב',
    body: `<p>לניה היא <strong>אוטומט תאי</strong> — רשת של תאים שכולם מצייתים לאותו
      כלל מקומי בו־זמנית. היא נצר ישיר ל"משחק החיים" של קונוויי (1970), שבו התגלה
      דבר מדהים: אוטומט תאי יכול להיות <strong>טיורינג־שלם</strong> — כלומר לחשב כל
      דבר שמחשב רגיל יכול, כולל להריץ את עצמו.</p>
      <p>לניה שייכת לתחום <strong>"חיים מלאכותיים" (Artificial Life)</strong>: לחקור
      את החיים לא בעזרת מיקרוסקופ, אלא על ידי בנייתם מאפס בתוך המחשב ובדיקה איזה
      כללים מולידים "חיים". מעניין במיוחד: היצורים של לניה התגלו ב<strong>חיפוש
      ממוחשב</strong> — המחשב סרק אינספור צירופי פרמטרים וזרעים כדי למצוא את המעטים
      ששורדים. האורביום לא תוכנן; הוא <em>נמצא</em>.</p>
      <p><strong>דוגמה ספציפית:</strong> כפתור "🎲 אקולוגיה אקראית" עושה בדיוק את זה
      בזעיר אנפין: הוא מגריל חוקים, מריץ בדיקה חבויה של 60 צעדים, וזורק את מה שמת.
      זה אלגוריתם חיפוש — אתם רואים "אבולוציה מלאכותית" בפעולה.</p>
      <p class="help-try">🤔 למחשבה: אם אפשר "לחשב" יצור חי, האם המוח שלכם הוא בסך
      הכול מחשב ביולוגי? ואם כן — האם תוכנה יכולה להיות חיה באמת, או רק "כאילו"?</p>`,
  },
  {
    emoji: '✨', name: 'פילוסופיה ותיאולוגיה',
    body: `<p>בעולם הזה "נבראים" יצורים מתוך <strong>חוק אחד</strong>, בלי שאף יד ציירה
      אותם. זה נוגע בשאלה עתיקה: האם סדר ומורכבות מחייבים "מתכנן", או שהם יכולים
      לצוץ מעצמם מתוך כללים פשוטים? לניה היא הדגמה חיה לצד השני של הוויכוח —
      <strong>סדר ספונטני</strong>.</p>
      <p>מרתק במיוחד ה<strong>"כוונון העדין"</strong>: החיים אפשריים רק ברצועה צרה
      להפליא של פרמטרים. קצת יותר מדי σ — כאוס. קצת פחות — מוות. חלון החיים דק כמו
      חוט. יש הרואים בכך מטאפורה יפה, ויש שמקשרים זאת לטענה בקוסמולוגיה שגם קבועי
      הטבע ביקום שלנו "מכוונים" באופן מדויק להפליא שמאפשר חיים.</p>
      <p><strong>דוגמה ספציפית:</strong> הזיזו את σ לאט מ‑0.017 כלפי מעלה וצפו במחוון:
      ירוק → אדום קורה כמעט בבת אחת. אין "אזור אפור" נדיב. הגבול בין קיום לאי‑קיום
      חד ומדויק — וזה עצמו נושא לדיון.</p>
      <p class="help-try">🤔 למחשבה: כל תא כאן מציית לחוק דטרמיניסטי לחלוטין — אין בו
      שום בחירה. ובכל זאת האורביום נראה כאילו הוא "רוצה" לשחות ולשרוד. אם התנהגות
      תכליתית יכולה לנבוע מחוק עיוור, מה זה אומר על הרצון החופשי שלנו?</p>`,
  },
  {
    emoji: '⚛️', name: 'פיזיקה',
    body: `<p>מפת אזור החיים שבממשק היא בדיוק <strong>דיאגרמת מופעים (phase diagram)</strong> —
      אותו כלי שבו פיזיקאים מתארים מתי מים הם קרח, נוזל או אדים. כאן ה"מופעים" הם
      דעיכה (כחול), חיים (ירוק) וכאוס (אדום), והקווים ביניהם הם
      <strong>מעברי פאזה</strong>.</p>
      <p>"קצה הכאוס" הוא <strong>נקודה קריטית</strong> — כמו הטמפרטורה המדויקת שבה
      חומר משנה מצב. בפיזיקה, בנקודות קריטיות קורים דברים יוצאי דופן: תנודות בכל
      קנה מידה, וארגון עצמי. הרעיון ש<strong>מערכות מורכבות "מכווננות את עצמן" לקצה
      הזה</strong> (self-organized criticality) מסביר מפולות שלג, רעידות אדמה,
      ופעילות מוחית.</p>
      <p><strong>דוגמה ספציפית:</strong> נסו את "העולם הנושם 🫁" והסתכלו בגרף המסה.
      העולם כולו פועם במחזוריות יציבה — זו "תנודה" קולקטיבית של אלפי תאים שמסתנכרנים
      בלי מנצח, בדיוק כמו תופעות תהודה בפיזיקה.</p>
      <p class="help-try">🤔 למחשבה: למה דווקא ב"קצה" בין סדר מוחלט לכאוס מוחלט קורים
      הדברים המעניינים? יש הטוענים שגם המוח, מושבות נמלים ואפילו שווקים כלכליים חיים
      על קצה כזה. במה זה יתרון?</p>`,
  },
  {
    emoji: '🧠', name: 'חקר המוח',
    body: `<p>העולם "הגחליליות ✨" בנוי משני חוקים בו־זמנית: <strong>עידוד מקרוב ודיכוי
      מרחוק</strong>. זו בדיוק הארכיטקטורה של המוח — נוירון מעורר את שכניו הקרובים
      ומעכב את הרחוקים (מנגנון שנקרא "עיכוב צדי", lateral inhibition), וכך נוצרים
      ניגודיות, חדות ודפוסים.</p>
      <p>אותו עיקרון מדויק — עידוד קצר‑טווח, דיכוי ארוך‑טווח — הוא הנוסחה של
      <strong>דפוסי טיורינג</strong> (אלן טיורינג, 1952). זה מה שיוצר פסים על זברה,
      כתמים על נמר, נקודות על דג, וספירלות על קונכייה. אותה מתמטיקה שמייצרת מחשבה
      במוח מייצרת גם את הדוגמאות על עורות של חיות.</p>
      <p><strong>דוגמה ספציפית:</strong> בגחליליות, כל "ניצוץ" נדלק כשמספיק שכנים
      קרובים פעילים, ונכבה כשהאזור סביבו נעשה צפוף מדי — בדיוק כמו נוירון שיורה
      ואז נכנס ל"תקופת מנוחה". התוצאה היא הבהוב שלא נגמר, כמו גלי פעילות במוח ער.</p>
      <p class="help-try">🤔 למחשבה: אותו חוק פשוט מסביר גם דפוס על גב חיה, גם חישוב
      במוח, וגם ריצוד על המסך. למה הטבע "ממחזר" את אותו רעיון שוב ושוב בכל כך הרבה
      מקומות?</p>`,
  },
  {
    emoji: '🎨', name: 'אמנות',
    body: `<p>לניה היא <strong>אמנות גנרטיבית</strong>: יצירה שנוצרת לא כשאמן מצייר כל
      פרט, אלא כשהוא כותב כלל — והכלל "מגדל" את היצירה. האמן שולט בחוקים, אבל
      התוצאה מפתיעה גם אותו. זו זרם אמנות ממשי, מ‑Sol LeWitt ועד אמני קוד בני זמננו,
      שנמכר בגלריות ובמכירות פומביות.</p>
      <p>בלניה יש שפה חזותית עשירה: הפלואידיות האורגנית של התנועה, פלטות הצבע
      השונות, והמרקמים ההיפנוטיים של "שדה הגלים 🌊" שנראים כמו טביעת אצבע חיה או
      אלמוגים. היופי כאן אינו מקרי — הוא נובע מאותם עקרונות של איזון וסימטריה
      שהעין האנושית מוצאת מושכים גם בטבע.</p>
      <p><strong>דוגמה ספציפית:</strong> החליפו את פלטת הצבע ל"תרמי 🔥" בזמן שרץ
      "שדה הגלים", ותנו לו להתייצב. אותה פיזיקה בדיוק נראית כמו יצירת אמנות אחרת
      לגמרי. הצבע אינו קישוט — הוא חלק מהמשמעות.</p>
      <p class="help-try">🤔 למחשבה: אם המחשב יצר דבר יפה שאיש לא תכנן במדויק — של מי
      היצירה? של מי שכתב את החוק? של המקריות שבחרה את הזרע? או של הצופה שקורא בזה
      יופי?</p>`,
  },
  {
    emoji: '🌍', name: 'פילוסופיה של המדע',
    body: `<p>לניה היא <strong>מודל</strong> — ייצוג פשוט ומכוון של משהו מורכב. כל המדע
      עובד ככה: אנחנו לא חוקרים את המציאות במלואה, אלא בונים "צעצוע" שתופס את העיקר
      ומתעלם מהשאר. מודל טוב הוא לא זה שהכי מדויק — אלא זה שהכי <em>מלמד</em>.</p>
      <p>כאן נחשפת מתיחות עמוקה: היצורים של לניה אינם חיים אמיתיים, ובכל זאת הם
      מלמדים אותנו משהו אמיתי על מה שחיים <em>יכולים</em> להיות בעיקרון. זו הסיבה
      שמדענים קוראים ללניה "ביולוגיה של חיים אפשריים" — היא חוקרת לא רק את החיים
      שיש, אלא את כל החיים שהיו יכולים להיות תחת חוקים אחרים.</p>
      <p><strong>דוגמה ספציפית:</strong> מפת אזור החיים מסומנת בממשק במפורש כ"קירוב"
      ולא כאמת מוחלטת — כי היא נדגמת בקנה מידה אחד ואילו העולם רץ באחר. זו יושרה
      מדעית: מודל טוב מצהיר בגלוי היכן הוא מקורב והיכן לא לסמוך עליו.</p>
      <p class="help-try">🤔 למחשבה: מה מודל יכול ללמד אותנו שהמציאות עצמה לא יכולה?
      ומאיזה רגע מודל "פשוט מדי" הופך ממועיל למטעה — כשמתחילים להאמין שהצעצוע הוא
      העולם?</p>`,
  },
];
